...
# Load .env (same convention as citation/network.py) so HF_TOKEN is available
# for gated repos (e.g. google/gemma-3-12b-it) without hardcoding it here.
try:
...
class FixedTripleExtractor:
    ...
    def __init__(
        ...
        # Qwen3 emits <think> blocks and understands the /no_think control
        # token; other model families (e.g. Gemma3) don't — treating
        # "/no_think" as literal user text would contaminate the prompt.
    ...
    def _load(self):
        ...
        try:
            ...
            # Some checkpoints (e.g. gemma-3-*-it) are natively multimodal
            # (Gemma3ForConditionalGeneration) and aren't registered under
            # AutoModelForCausalLM even though we only use them for text.
            from transformers import AutoModelForImageTextToText
            logger.info(
                f"{self.model_name} is not a plain causal LM — "
                f"loading as AutoModelForImageTextToText (text-only usage)."
            )
            model = AutoModelForImageTextToText.from_pretrained(self.model_name, **model_kwargs)
        ...
        if self._is_multimodal:
            # The "text-generation" pipeline doesn't accept image-text-to-text
            # model classes — drive tokenize/generate/decode manually instead.
            self._pipeline = None
            self._model = model
        else:
    ...
    def _extract_paragraph(
        ...
        user_content = _make_fixed_user_prompt(
        ...
        try:
            if self._is_qwen3:
                ...
                try:
                    ...
                    # Some chat templates (e.g. older Gemma) reject a
                    # standalone "system" role — fold it into the user turn.
                    merged = [{
                        "role": "user",
            ...
            if self._is_multimodal:
                # "text-generation" pipeline doesn't support image-text-to-text
                # model classes — tokenize/generate/decode manually instead.
                import torch
